/*
  This file is property of ashishtank
  Github: https://github.com/ashishtank/MMM-MercedesMe
*/
...
class OAuth {
  ...
  getAuthorizeUrl() {
    ...
    const authorizationUrl =
      this.authUrl +
      `?response_type=code&client_id=${this.client_id}&&redirect_uri=${this.redirectUri}&scope=${this.scope}&state=${this.state}`;
  ...
  refreshToken(cb) {
    ...
    const refreshUrl =
      this.tokenUrl +
      `?grant_type=refresh_token&redirect_uri=${this.redirectUri}`;
    const options = {
      url: refreshUrl,
      method: "POST",
      headers: {
  ...
  isExpired() {
    if (isNullOrUndefined(this.access_token) || this.access_token.length === 0)
      return true;
